...
# ================== CONFIG & STYLE (V15.0 - SAME AS V12.7 LOGIC) ==================
...
def fetch_egx_data(symbol=None, scan_all=False):
    ...
    if scan_all:
        ...
        # تعديل جوهري: البحث بـ match لضمان ظهور FERC
        payload = {
...
def analyze_stock(d_row):
    try:
        d = d_row['d']
        # ترتيب البيانات ثابت لضمان دقة الحسابات
        ...
        vol_txt, vol_col = ("🔥 زخم انفجاري", "#ffd700") if ratio > 1.3 else ("⚪ تداول هادئ", "#8b949e") if ratio > 0.7 else ("🔴 سيولة غائبة", "#ff4b4b")
        rec, col = ("🛑 تشبع شراء", "#ff4b4b") if rsi_val > 72 else ("💎 صفقة ذهبية", "#ffd700") if is_gold else ("🚀 شراء قوي", "#00ff00") if t_score >= 75 else ("⚖️ انتظار", "#58a6ff")
...
def render_stock_ui(res, title=""):
    ...
    # --- خطة السيولة بالأسعار ---
    st.markdown("---")
    st.subheader("🛠️ خطة السيولة وتمركزات التعزيز")
    budget = st.number_input("ميزانية السهم (ج):", value=20000, key=f"plan_{res['name']}")
    ...
    <div class='plan-container'>
        <div class='plan-step up-line'>
            ...
            - التعزيز النهائي: الـ {p3:,.0f} ج الباقية عند تجاوز {res['s_t']:.2f}.
        </div>
        <div class='plan-step down-line'>
            <b>📉 سيناريو الهبوط:</b><br>
            - لو نزل لـ <b>{res['s_s']:.2f}</b>، استهلك الـ {p2:,.0f} ج لتعديل المتوسط.<br>
            - التعزيز النهائي: الـ {p3:,.0f} ج المتبقية فور العودة لـ {res['p']:.2f}.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.page == 'home':
    ...
    sym = st.text_input("ادخل رمز السهم (مثلاً: FERC)").upper().strip()
    if sym:
    ...
    if st.button("🔍 فحص السوق"):
        results = [an for r in fetch_egx_data(scan_all=True) if (an := analyze_stock(r)) and an['t_score'] >= 70]
    ...
    p_now = c3.number_input("السعر الجديد", 0.0)
    if p_old > 0 and q_old > 0 and p_now > 0:
        for label, q in [("نص الكمية (0.5x)", int(q_old*0.5)), ("نفس الكمية (1:1)", q_old), ("ضعف الكمية (2:1)", q_old*2)]:
            ...
            st.markdown(f"<div class='avg-card'><b>{label}</b>: شراء {q:,} سهم<br>المتوسط: <span class='price-callout'>{avg:.3f}</span></div>", unsafe_allow_html=True)
